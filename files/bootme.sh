#!/usr/bin/env bash
#
# This script installs puppet 3.x or 4.x and deploy the manifest using puppet apply -e "include role_lb"
#
# Usage:
# Ubuntu / Debian: wget https://raw.githubusercontent.com/relybv/dirict-role_lb/master/files/bootme.sh; bash bootme.sh
#
# Red Hat / CentOS: curl https://raw.githubusercontent.com/relybv/dirict-role_lb/master/files/bootme.sh -o bootme.sh; bash bootme.sh
...
# get or update repo
if [ -d /root/role_lb ]; then
  echo "Update repo"
  cd /root/role_lb
  git pull
else
  echo "Cloning repo"
  git clone https://github.com/relybv/dirict-role_lb.git /root/role_lb
  cd /root/role_lb
fi

# install puppet
bash /root/role_lb/files/bootstrap.sh $PUPPETMAJOR
...
/usr/local/bin/puppet apply -e "include role_lb" --write-catalog-summary --log_level=err
